...
def get_vector_store():
    duckdb_conn = duckdb.connect(database=vector_db_path,
    config={
...
@click.option('--max-toots-per-handle', default=1000, help='number of toots to pull per handle (default: 1000)')
@click.option('--handles', default="", help='comma separated handle list (for example, "@me@mysite.com, @me@frensite.com"), this overwrites handles in the config file (default: "")')
    ...
    for handle in handles_from_args or config.get("handles", []):
        ...
        while True:
            with sqlmodel.Session(engine) as session:
                data = mastodon_client.account_statuses(username, limit=page_size, max_id=max_id)
                cnt += len(data)
                max_id = data[-1]["id"]
                for toot in data:
                    ...
                    if not text:
                        continue
                    statement = sqlmodel.select(Text).where(Text.content == text).where(Text.handle == handle)
...
def chat(verbose):
    ...
    messages.append((resp.response_metadata["message"]["role"], resp.content,))
